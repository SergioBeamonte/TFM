...
\begin{tikzpicture}[
    ...
    ellipse_node/.style={ellipse, draw=blue!50, fill=cyan!10, minimum width=2.5cm, minimum height=1cm},
    rect_node/.style={rectangle, draw=black, fill=orange!30, minimum width=3cm, minimum height=1cm},
    hex_node/.style={regular polygon, regular polygon sides=6, draw=black, fill=purple!30, minimum width=3cm, minimum height=1cm, rotate=90},
    arrow/.style={-{Stealth[scale=1.2]}, draw=blue!50!black},
    dashed_arrow/.style={-{Stealth[scale=1.2]}, draw=blue!50!black, dashed}
]
    \node[ellipse_node] (HD) {HEARTDISEASE};
    \node[ellipse_node, below left=1cm and 1cm of HD] (ANG) {ANGIOGRAM};
    \node[ellipse_node, right=2cm of HD] (PAIN) {PAIN};
    \node[ellipse_node, below left=1.5cm and 1cm of ANG] (ER) {EARLYRESULTS};
    \node[rect_node, below left=1.5cm and 0.5cm of ER] (HS) {HEARTSURGERY};
    \node[rect_node, below right=1.5cm and 0.5cm of ANG] (HP) {HEARTPHARMA};
    \node[ellipse_node, below=1cm of HS] (LQ) {LIFEQ};
    \node[ellipse_node, below=1cm of HP] (EC) {ECONOMICCALC};
    \node[hex_node, below=1.5cm of EC] (UT) {UTILITY};

    \draw[arrow] (HD) -- (ANG);
    \draw[arrow] (HD) -- (PAIN);
    \draw[arrow] (ANG) -- (ER);
    \draw[dashed_arrow] (ANG) -- (HP);
    \draw[dashed_arrow] (PAIN) -- (HS);
    \draw[dashed_arrow] (PAIN) -- (HP);
    \draw[arrow] (HS) -- (ER);
    \draw[dashed_arrow] (ER) -- (HS);
    \draw[arrow] (HS) -- (LQ);
    \draw[arrow] (HS) -- (EC);
    \draw[arrow] (HP) -- (EC);
    \draw[arrow] (LQ) -- (UT);
    \draw[arrow] (EC) -- (UT);
    \draw[dashed_arrow] (ER) -- (HP);
\end{tikzpicture}
