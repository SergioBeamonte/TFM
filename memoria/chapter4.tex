% ==============================================================
%  Chapter 4 -- Estimation of the influence diagram parameters
% ==============================================================

\chapter{Estimation of the influence diagram parameters}
...
\section{Parameter tuning system design}
...
a target decision are kept. 

The result is a list of triples:
\[
  (\text{evidence},\; \text{decision},\; \text{expected action})
\]
that the scoring step will check one by one.

\subsection{A real rule from the bypass model}
\label{sec:bypass-example}

Throughout the rest of the chapter we ground the description on the
\emph{bypass} influence diagram already introduced in
Chapter~\ref{ch:framework} (Figure~\ref{fig:bypass-id},
Tables~\ref{tab:bypass-cpt}--\ref{tab:bypass-eu}). Here we focus on
the operational side that Chapter~\ref{ch:framework} does not
discuss: how a single line of the expert rule base is turned into a
constraint the optimiser can check.

The rule base is a plain text database in CSV format with one row per rule and one column per
...
\begin{center}
\texttt{ANGIOGRAM, EARLYRESULTS, ECONOMICALC, HEARTDISEASE,\\
...
explicit rule
\begin{quote}
\emph{If \texttt{Angiogram}~$=$~\textsc{negative} \;and\;
\texttt{EarlyResults}~$=$~\textsc{CoRem} \;and\;
\texttt{Pain}~$=$~\textsc{absent},
\quad then \;\texttt{HeartPharma}~$=$~\textsc{no}.}
\end{quote}
...
Shachter on \texttt{HeartPharma} and check that its optimal action is
...
$\boldsymbol{\phi}$ that lives in the whole real space: its entries are only
initialised uniformly within symmetric bounds — in our runs $[-5,5]$ for the CPT
entries and $[-10,10]$ for the utilities — and are then sampled freely over
$\mathbb{R}$, while a \emph{decoder} turns that raw vector into a valid
...
\begin{itemize}
  ...
  \item \textbf{Utility node --- sigmoid.} Each free utility entry is the
        ...
        by the expert. Because the sigmoid is
        asymptotic --- it maps any finite raw value into the \emph{open}
        interval $(0,1)$ --- the decoded utilities stay strictly inside the
        interior of the $[u_{\min},u_{\max}]$ range, i.e.\ they never reach
        exactly $u_{\min}$ nor $u_{\max}$ except at the two pinned entries.
\end{itemize}
...
compared experimentally in Chapter~\ref{ch:experiments}. The simplest member is the
...
\[
   ...
   \mathbf{1}\!\left[\delta^{*}_{i_l}\!\left(\boldsymbol{\Theta}
...
% ==============================================================
\section{The EDA based optimizer}
% ==============================================================

\textsc{EDAspy}~\cite{soloviev2024edaspy} is the reference open-source
EDA library of the UPM Computational Intelligence
Group (CIG)\footnote{\url{https://cig.fi.upm.es/}}; it implements the univariate and
...
\end{itemize}
Together they form a spectrum from fully factorized to fully
...
choice to the empirical comparison of Chapter~\ref{ch:experiments}. Richer variants such
...
bounds. This choice is deliberate. Because the bounds are centered at
